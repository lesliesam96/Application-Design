...
function renderLiveMatchData(matchData) {
    ...
    liveMatchDataElement.innerHTML = `
        <h2>${matchData.event_home_team} <small class="text-muted">(HOME)</small> vs. ${matchData.event_away_team} <small class="text-muted">(AWAY)</small></h2>
        <h2>Score : ${homegoal} - ${awaygoal}</h2>
        <hr>
        <dl class="row">
